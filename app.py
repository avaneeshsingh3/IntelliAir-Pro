...
def normalize_hospital_docs(docs, lat, lon):
    rows = []
    for doc in docs:
    ...
    return pd.DataFrame(rows).sort_values(["distance_km", "rating"], ascending=[True, False]).head(5)
...
tabs = st.tabs(["Overview", "Prediction", "Hospitals & Map"])
...
with tabs[0]:
    ...
    with t1:
        ...
        trend_fig.update_layout(template="plotly_dark" if selected_theme == "Dark" else "plotly_white", height=360)
        st.plotly_chart(trend_fig, use_container_width=True)
    with t2:
        comp_fig = px.bar(comparison_df.head(10), x="City", y="AQI", color="AQI", title="City Comparison", color_continuous_scale="Turbo")
        comp_fig.update_layout(template="plotly_dark" if selected_theme == "Dark" else "plotly_white", height=360)
